...
void swoole_init(void)
{
    ...
    SwooleG.cpu_num = SW_MAX(1, sysconf(_SC_NPROCESSORS_ONLN));//获取cpu数量
    SwooleG.pagesize = getpagesize(); //获取系统分页大小
    //get system uname
    uname(&SwooleG.uname); // 内核信息
    //random seed
    srandom(time(NULL)); // 随机数播种
    ...
    {
        SwooleG.max_sockets = MAX((uint32_t) rlmt.rlim_cur, 1024); // 最小1024, 最大1024*1024
        SwooleG.max_sockets = MIN((uint32_t) rlmt.rlim_cur, SW_SESSION_LIST_SIZE);
    }

    SwooleG.socket_buffer_size = SW_SOCKET_BUFFER_SIZE; // 256k
	...
	// 设置task临时目录为/tmp/swoole.task.XXXXXX
