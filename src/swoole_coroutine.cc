...
ZEND_BEGIN_ARG_INFO_EX(arginfo_swoole_coroutine_dnsLookup, 0, 0, 1)
    ZEND_ARG_INFO(0, domain_name)
    ZEND_ARG_INFO(0, timeout)
ZEND_END_ARG_INFO()
...
{
    uint32_t size = SW_DEFAULT_PHP_STACK_PAGE_SIZE; // 8k
...
void PHPCoroutine::main_func(void *arg)
{
    ...
    do {
        ...
        call = zend_vm_stack_push_call_frame(call_info, func, argc, object_or_called_scope); // 将closure参数压栈
    } while (0);
#endif
	// 传递参数
    for (i = 0; i < argc; ++i)
    ...
    save_vm_stack(task); // 保存当前vmstack信息到task上, task在vm_stack上面
    record_last_msec(task);
...
#endif
        zend_execute_ex(EG(current_execute_data)); // 执行closure
    }
...
long PHPCoroutine::create(zend_fcall_info_cache *fci_cache, uint32_t argc, zval *argv)
{
    ...
    save_task(get_task()); // 初始化main_task, 第一次执行时, 获取的是main_task(就是初始化好的空task), 将EG执行栈位置, execute_data保存在该task上
